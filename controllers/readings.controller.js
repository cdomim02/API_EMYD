    ...
    try {
        ...
        if (!readings.length) {
            return res.status(404).json({
                msg: 'No se encontraron lecturas para este sensor'
            });
        }
        res.status(200).json(readings);
    } catch (err) {
        res.status(500).json({
            msg: 'Error al obtener las lecturas', error: err.message
        });
    }
};
...
const deleteReadingsBySensor = async (req, res) => {
    try {
        ...
        if (result.deletedCount === 0) {
            return res.status(404).json({
                msg: "No se encontraron lecturas para eliminar"
            });
        }
        res.status(200).json({
            msg: "Todas las lecturas del sensor fueron eliminadas"
        });
    } catch (err) {
        res.status(500).json({
            msg: "Error al eliminar las lecturas", error: err.message
        });
    }
};
